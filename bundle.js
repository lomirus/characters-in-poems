...
const shijing = JSON.parse(await Deno.readTextFile('./poetry/shijing.json'));
const chuci = JSON.parse(await Deno.readTextFile('./poetry/chuci.json'));
const tangshi300 = JSON.parse(await Deno.readTextFile('./poetry/tangshi300.json'));
const songci300 = JSON.parse(await Deno.readTextFile('./poetry/songci300.json'));
const tangshi = JSON.parse(await Deno.readTextFile('./poetry/tangshi.json'));
const songci = JSON.parse(await Deno.readTextFile('./poetry/songci.json'));
const poetry = {
    shijing,
    chuci,
    tangshi300,
...
function getCharPinyin(__char) {
    return dict[__char];
}
function getParagraphPinyin(words) {
    return words.split("").map(getCharPinyin);
}
function getParagraphCount(paragraph, keyword) {
    const paragraphPinyin = getParagraphPinyin(paragraph.replace(/[，。！？\[\]]/g, ""));
    const keywordPinyin = getParagraphPinyin(keyword);
    return keywordPinyin.reduce((total, keyword1)=>paragraphPinyin.includes(keyword1) ? total + 1 : total
    , 0);
}
function getMaxCountLine(poem, keyword) {
    let maxCount = 0;
    let maxCountLine = {
        title: "",
        author: "",
...
function getMaxCountLines(poems, keyword) {
    let maxCounts = [];
    ...
    return [
        maxCount,
        maxCounts.map(([_, paragraph])=>paragraph
        )
    ];
}
function highlightByColor(paragraph, keyword) {
    return paragraph.split("").map((__char)=>getParagraphPinyin(keyword).includes(getCharPinyin(__char)) ? `\x1b[32m${__char}\x1b[0m` : __char
    ).join("");
}
function highlightByBrackets(paragraph, keyword) {
    return paragraph.split("").map((__char)=>getParagraphPinyin(keyword).includes(getCharPinyin(__char)) ? `[${__char}]` : __char
    ).join("");
}
...
const specPoetry = (()=>{
    let list = (args.p ? args.p.split(",") : []).concat(args.poetry ? args.poetry.split(",") : []);
    ...
    return list.reduce((poetries, poetryName)=>{
...
maxCountLines.forEach((line)=>{
    line.paragraph = specFormat === "color" ? highlightByColor(line.paragraph, specWord) : highlightByBrackets(line.paragraph, specWord);
